...
- **Logo.** The CNE lettermark: C, N and E drawn as one system on a 420 x 160 grid, stroke 22, every turn 45 or 90 degrees, every join and cap round. The E's middle arm carries the lens accent, which is the layer inside the letter. Geometry lives in `scripts/mark-geometry.mjs` and nowhere else.
- **Mark.** The chamfered C alone with a lens core, square, on a 140 grid, stroke 20. Use it for the favicon, avatar and app icon, and anywhere the lettermark has no horizontal room. Never put a horizontal bar inside the C: it reads as a euro sign.
