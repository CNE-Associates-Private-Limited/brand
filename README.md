...
- **Logo.** The CNE lettermark chosen from the Recraft round (round1-r3) and redrawn as exact geometry: cap 100, stroke 22. The C is a true semicircle on the left with 45-degree terminals forming a bracket on the right; the N's stem is chamfered at 45 degrees to slot into that bracket with a constant gap; the N's diagonal fuses into the E's spine; all three E arms are cut at 45 degrees in the same direction, the middle one in lens blue. Soft C, hard N and E. Geometry lives in `scripts/mark-geometry.mjs` and nowhere else; `dist/mark-polygons.json` feeds the 3D pipeline from it with the arcs sampled.
- **Mark.** The same C alone with a lens core, square, on a 140 grid. Use it for the favicon, avatar and app icon, and anywhere the lettermark has no horizontal room. Never put a bar inside the C: it reads as a euro sign.
